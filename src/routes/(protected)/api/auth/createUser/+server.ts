...
import type { newCustomerRecord, newUserRecord } from '$lib/types';
import { json, type RequestHandler } from '@sveltejs/kit';

export const POST: RequestHandler = async ({ request }) => {
	try {
		const data: newUserRecord = await request.json();
		...
		if (result.acknowledged) {
			...
			return json(
				{ success: false, message: 'Error creating account. Try again later.' },
				{ status: 201 }
			);
		}
	} catch (error) {

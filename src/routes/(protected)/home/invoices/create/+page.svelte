<script lang="ts">
	import type { BaseInvoiceRecord, CustomerRecord, ServiceRecord } from '$lib/types';
	...
	let invoice: BaseInvoiceRecord = $state({
	...
	let tempServiceItems = $state([]) as { serviceID: number; quantity: number }[];
...
<div class="content">
	<form onsubmit={onSave}>
		...
		<div class="secondHeader">
			<div>
				<b> Service Date: </b>
				<input type="date" name="Date" id="" bind:value={invoice.date}  required />
			</div>
			<div>
				<b> Payment Due Date: </b>
				<input type="date" name="Date" id="" bind:value={invoice.dueDate} d required />
			</div>
			<div>
				<b>Payment Recieved</b>
				<input type="checkbox" checked={invoice.paid === 1} onchange={handlePaidChange} />
			</div>
		</div>
		...
		<div class="billTo">
			<div class="billee">
				<b> Billed To: </b>
				<select name="" id=""  bind:value={invoice.customerID} required>
		...
		<div class="services">
			...
			<button
				type="button"
				style="font-size:.9rem; padding: .5rem; grid-column: 1;"
				onclick={() => {
					tempServiceItems.push({ serviceID: 0, quantity: 0 });
				}}>Add Service</button
			>
		</div>
	</form>
...
{#snippet service(serviceID: number, quantity: number, i: number)}
	...
	<div style="grid-column: 1;">
		<select name="" id=""  bind:value={tempServiceItems[i].serviceID} required>
	...
	<div style="grid-column: 3;">
		<input type="number" style="width: 3rem;" name="" id="" bind:value={tempServiceItems[i].quantity}/>
	</div>
	...
	<div style="grid-column: 6; color: red; cursor:pointer">X</div>
{/snippet}
